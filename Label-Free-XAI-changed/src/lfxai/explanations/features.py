...
class AuxiliaryFunction_v2(Module):
    def __init__(self, black_box: Module, base_features: torch.Tensor, labels: torch.Tensor, vector_directions: torch.Tensor) -> None:
        super().__init__()
        self.black_box = black_box
        self.base_features = base_features
        self.prediction = black_box(base_features)
        self.labels = labels
        self.directions = torch.stack([vector_directions[labels[k]] for k in range(labels.shape[0])])

    def forward(self, input_features: torch.Tensor) -> torch.Tensor:
        if len(self.prediction) == len(input_features):
            return torch.sum((self.black_box(input_features) - self.prediction) * self.directions, dim=-1)
        elif len(input_features) % len(self.prediction) == 0:
            n_repeat = int(len(input_features) / len(self.prediction))
            return torch.sum(
                (self.black_box(input_features) - self.prediction.repeat(n_repeat, 1)) * self.directions.repeat(n_repeat, 1),
                dim=-1,
            )
        else:
            raise ValueError(
...
def attribute_auxiliary_v2(
    encoder: Module,
    data_loader: torch.utils.data.DataLoader,
    device: torch.device,
    attr_method: Attribution,
    vector_directions: torch.Tensor, ###line added 
    baseline=None,
) -> np.ndarray:
    attributions = []
    ...
    for inputs, labels in tqdm(data_loader):
        inputs = inputs.to(device)
        # auxiliary_encoder = AuxiliaryFunction(encoder, inputs)
        auxiliary_encoder = AuxiliaryFunction_v2(encoder, inputs, labels, vector_directions)
        attr_method.forward_func = auxiliary_encoder
        if isinstance(attr_method, Saliency):
            attributions.append(attr_method.attribute(inputs).detach().cpu().numpy())
        else:
